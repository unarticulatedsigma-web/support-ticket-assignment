...
**Pending assignment accountability and auto-retry.** For `assignment_pending` tickets, the team lead is the accountable party — responsible for noticing the ticket if it isn't resolved automatically, but not the working-agent owner. Resolution doesn't require the lead to act in the common case: the system automatically re-attempts assignment for a company's pending tickets whenever a relevant event occurs — an agent's active load decreases (a ticket closes) or an agent's schedule window opens (a shift starts).

Event-triggered retry was chosen over immediately routing to "the next scheduled agent" because it reuses the same current-load fairness comparison as every other ticket, and reacts to the actual condition that makes a pending ticket resolvable, rather than guessing from the schedule alone. The trade-off: it depends on those two events firing reliably, and won't catch eligibility changes that happen outside them — for example, a lead manually raising an agent's capacity cap or editing their schedule directly. To guarantee no ticket can be stranded indefinitely because of a missed or uncovered event, the system also periodically reconciles: any ticket still `assignment_pending` past a bounded interval is re-checked regardless of what triggered it, so the maximum pending time is always bounded, not open-ended. (Reconciliation interval and retry-trigger internals: see `implementation.md`.)

Each re-attempt is safe to run repeatedly, since it only acts while the ticket remains unassigned — it can never double-assign or touch a ticket that already has an owner. If multiple pending tickets exist for a company, they're re-attempted oldest-first, so the longest-waiting ticket gets first claim on newly freed capacity. A caller explicitly retrying the same still-unassigned ticket also re-runs the check, as before. Once an eligible agent is found, that agent becomes the ticket's owner and `assignment_pending` clears. What's still not built is a designated backup/overflow agent as an alternate resolution path — event-triggered retry (backed by reconciliation) uses the same fairness algorithm as any other ticket, not a special-case routing rule.
...
- **No designated backup/overflow agent** — `assignment_pending` tickets resolve automatically via event-triggered retry (on agent load dropping or a shift starting) plus periodic reconciliation as a bounded safety net, reusing the standard fairness algorithm. What's not built is a separate routing path to a special backup/overflow agent as an alternative — that's a distinct design decision, out of scope for the trial.
- **Reconciliation interval left unspecified here** — the PRD guarantees pending tickets have a bounded maximum wait via periodic reconciliation, but the actual interval is a tunable parameter to be set in `implementation.md`, not a product-level requirement.
