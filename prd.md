...
**Assignment algorithm** (runs synchronously when the assignment API is called, per ticket):
...
**Resolving a conflict between two stated requirements.** The brief requires both "a new ticket should not be left without an owner" and, implicitly, that agents never get overloaded or assigned work they're not available for. Under normal operation there's no conflict — every ticket is assigned immediately to a real, available, under-capacity agent. But if zero eligible agents remain after the availability and capacity filters, those two requirements can't both hold exactly as stated, and the system has to pick one to prioritize: it does **not** silently assign the ticket to someone unavailable or over capacity just to satisfy "always owned," because a fabricated assignment would violate availability and capacity constraints instead. The system attempts immediate assignment, and if no eligible agent exists, the ticket enters `assignment_pending` and is retried automatically as availability/capacity changes (event-triggered retry, backed by periodic reconciliation — see below). This is an explicit, defined safety behavior — a deliberate resolution of a genuine conflict between two requirements — not a silent violation of either one. It's also distinct from a recurring coverage gap: a team can have continuous scheduled coverage and still temporarily hit zero eligible agents because everyone available is at their capacity cap — schedule coverage alone doesn't prevent capacity exhaustion. Note that automatic retry does not itself guarantee the ticket will eventually get an owner — see the accountability section below for what is and isn't guaranteed.
...
- Why both mechanisms, not reconciliation alone: a reconciliation-only design would delay every pending ticket by up to a full interval before it's even checked, even when it could resolve right away. Event triggers handle the common case immediately; reconciliation is a safety net for what triggers can't see, not a redundant second path.
...
- **No urgency/priority routing** — every ticket uses the same synchronous, weighted-load logic regardless of stated urgency, to avoid reintroducing "ticket sits unassigned" delays via a windowed-assignment scheme.
